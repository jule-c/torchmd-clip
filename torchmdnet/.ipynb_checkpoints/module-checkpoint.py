        ...
        if not self.hparams.use_clip and not self.hparams.use_cloob:
            ...
            with torch.set_grad_enabled(stage == "train" or self.hparams.derivative):
                ...
                mol_properties = torch.hstack([
                    ...
                    batch.eXC, batch.eXX, batch.mPOL, batch.mC6, batch.sRMSD, batch.vTQ.unsqueeze(1), batch.vIQ.unsqueeze(1), batch.vEQ.unsqueeze(1)
                ])
                ...
                labels_mol, labels_atom \
                    = self(batch.z, batch.pos, batch=batch.batch,
                           atom_properties=atomic_properties,
                           mol_properties=mol_properties,
                           )
