...
class EquivariantLinearProbing(OutputModel):
    def __init__(self, hidden_channels, allow_prior_model=True):
        super(EquivariantLinearProbing, self).__init__(allow_prior_model=allow_prior_model)
        self.output_network = nn.Linear(hidden_channels, 1)

        self.reset_parameters()

    def reset_parameters(self):
        nn.init.xavier_uniform_(self.output_network.weight)
        self.output_network.bias.data.fill_(0)
