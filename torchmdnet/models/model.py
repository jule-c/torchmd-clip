...
def create_model(args, prior_model=None, mean=None, std=None):
    ...
    if args['use_clip'] or args['use_cloob']:
        ...
        clip_model = CLIP(
            molecule_encoder=representation_model,
            atom_prop_encoder=atom_prop_encoder,
    ...
    if args["transfer_learning"]:
        ...
        ckpt = torch.load(args["pretraining_ckpt"], map_location="cpu")
        state_dict = {re.sub(r"^model\.", "", k): v for k, v in ckpt["state_dict"].items()}
        state_dict_ = {}
        for name, param in state_dict.items():
            if 'molecule_encoder' in name:
                temp_name = name.split('molecule_encoder.')[-1]
                state_dict_[temp_name] = param

        representation_model.load_state_dict(state_dict_)
